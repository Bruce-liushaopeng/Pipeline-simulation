...
class Sim:
    ...
    def assign_c1(self):
        ...
        assignee = self.assign_c1_count % 3
        self.assign_c1_count+=1
        if(assignee==0 and not self.bf_c1w3>=2 ):
            self.bf_c1w3+=1
            return 3
        if(assignee==2 and not self.bf_c1w2>=2):
            self.bf_c1w2+=1
            return 2
        else:
            self.bf_c1w1+=1
    ...
    def start(self):
        ...
        with open('state_tracking_improve.csv', 'w', newline='') as file:
